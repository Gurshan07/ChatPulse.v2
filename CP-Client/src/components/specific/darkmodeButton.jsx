...
export default function UseSwitchesCustom() {
    ...
    const handleToggleDarkMode = () => {
        const newDarkMode = !isDarkmode; // Calculate new value
        setIsDarkmode(newDarkMode);
        changeColors(uiColor1, uiColor2, newDarkMode); // Pass updated value
    };
    ...
    return (
        ...
        >
            <Box sx={{ overflow: 'auto', px: 5, display: 'flex', flexDirection: 'column', gap: 2 }}>
                ...
                <h2 style={{ color: darkmode ? 'white' : 'black' }}>Darkmode</h2>
                <MUISwitch checked={isDarkmode} onChange={handleToggleDarkMode}  />
            </Box>
        </Stack>
    )
}
...
function MUISwitch(props) {
    ...
    const { uiColor1} = useColor();
   
    return (
        <SwitchRoot className={clsx(stateClasses)}  >
            <SwitchTrack>
                <SwitchThumb className={clsx(stateClasses)} sx={{ bgcolor: uiColor1 }} />
            </SwitchTrack>
